...
//#region RENDERS PARA DATOS
export const renderColActive = (value: boolean) => (
   <div className="w-full text-center">
      <span className={`inline-flex items-center gap-1 text-xs font-semibold p-2 rounded-full ${value ? "bg-green-100 text-green-700" : "bg-red-100 text-red-600"}`}>
         {value ? <icons.Lu.LuCheck size={12} /> : <icons.Lu.LuX size={12} />}
         {value ? "Activo" : "Inactivo"}
      </span>
   </div>
);
//#endregion RENDERS PARA DATOS
...
export interface ActionItem {
   label: string;
   iconName?: string; // opcional, se puede usar con lucide
   tooltip?: string;
   handleOnClick: (obj?: any) => void;
   color?: string;
   permission?: boolean;
   multiple?: (objs?: any) => void; // para acciones múltiples (no usado en fila)
}
...
export interface PropsTable<T extends object> {
   ...
   getRowActions?: (row: T) => ActionItem[]; // NUEVO: acciones por fila
...
const CustomTableInner = <T extends object>(
   {
      ...
      paginate = [1000, 500, 100, 50, 25, 10, 5],
      actions,
      loading,
      error,
      headerActions,
   ...
   // ========== COLUMNA DE ACCIONES ==========
   // Renderizado de la columna de acciones (dentro de la tabla)
   const renderActionsCell = (row: T, rowId: string) => {
      const rowActions = getRowActions ? getRowActions(row) : [];
      const filteredActions = rowActions.filter((a) => a.permission !== false);
      if (filteredActions.length === 0 && !actions) return null;

      // Si se proporciona el prop `actions` (renderizado personalizado), se usa ese
      if (actions) return actions(row);

      // Construir el menú con DaisyUI
      return (
         <>
            ...
            <div className="dropdown dropdown-end">
               <button className="btn btn-ghost btn-sm" aria-label="Acciones" onClick={(e) => e.stopPropagation()}>
                  <icons.Fi.FiMoreHorizontal size={16} />
               </button>
               <ul className="dropdown-content menu p-2 shadow bg-base-100 rounded-box w-52 z-50">
                  <li className="menu-title">
                     <span>
                        {singularName}: {String(row[columns[0]?.field] ?? rowId).slice(0, 20)}
                     </span>
                  </li>
                  {filteredActions.map((action, idx) => (
                     <li key={idx}>
                        ...
                        >
                           {action.iconName && <span className={`pi ${action.iconName}`} />}
                           {action.label}
                        </button>
                     </li>
                  ))}
   ...
   // // Referencias para los OverlayPanels (uno por fila, o usar un único y reutilizarlo)
   // const opRefs = useRef<Map<string, OverlayPanel>>(new Map());

   // // Función para mostrar/ocultar el menú de acciones
   // const toggleMenu = (e: React.MouseEvent, rowId: string) => {
   //    const op = opRefs.current.get(rowId);
   //    if (op) op.toggle(e);
   // };

   // // Render de la columna de acciones
   // const renderActionsColumn = (row: T, rowId: string, idx: number) => {
   //    const items = actionItems?.(row) || [];
   //    const filtered = items.filter((a) => a.permission);
   //    if (filtered.length === 0 && !onToggleActive) return null;

   //    const nameValue = rowNameField ? String(row[rowNameField] ?? "") : "";
   ...
   //                   onChange={() => onToggleActive(rowId, !row[activeField])}
   //                />
   //             </label>
   //          )}
   //          <ButtonPrime
   ...
   //          <OverlayPanel
   //             ref={(el) => {
   //                if (el) opRefs.current.set(rowId, el);
   //                else opRefs.current.delete(rowId);
   //             }}
   //             dismissable
   //             showCloseIcon={false}
   //             className="p-0 menu-context-datatable"
   //          >
   //             <div
   //                className="p-menu"
   //                style={{
   ...
   //                {nameValue && (
   //                   <>
   //                      <div className="p-submenu-header text-base font-bold px-3 py-2">
   //                         {singularName}: {nameValue.substring(0, 20)}
   //                         {nameValue.length > 20 ? "…" : ""}
   //                      </div>
   ...
   // Agregar la columna de acciones en la tabla (solo si hay actionItems o onToggleActive)
   // const hasActionsColumn = Boolean(actionItems || onToggleActive);
   const hasActionsColumn = Boolean(actions || getRowActions);
   ...
   const renderTable = () => {
      ...
      // const totalColumns = (showExpandCol ? 1 : 0) + normalColumns.length + (hasActionsColumn ? 1 : 0) + (showActionsCol ? 1 : 0);
            ...
            <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: 0 }}>
               <thead>
                  <tr>
                     ...
                     {(showActionsCol || actions || getRowActions) && (
                        <th
                           style={{
