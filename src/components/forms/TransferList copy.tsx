...
const CustomList: React.FC<CustomListProps> = memo(
   ({ title, items, search, setSearch, checked, data, disabled, sx, heightList, labelLeft, onRefetch, isRefetching, isLoading, handleToggleAll, handleToggle }) => {
      ...
      const allChecked = useMemo(() => {
         ...
         return numberOfChecked === items.length && items.length !== 0;
...
const TransferList: React.FC<TransferListProps> = ({
   ...
   // Efecto para sincronización - Detecta cambios en data y reinicializa
   useEffect(() => {
      console.log("🚀 ~ TransferList ~ data:", data);
      const availableIds = data.map((d) => d.id);
      const previousIds = left.concat(right);

      // Verificar si hay cambios significativos en los datos disponibles
      const idsChanged = previousIds.length !== availableIds.length || !previousIds.every((id) => availableIds.includes(id));

      // Si los datos han cambiado significativamente, reinicializar completamente
      if (idsChanged && data.length > 0) {
         setLeft(availableIds);
         setRight([]);
         setChecked([]);
         formik.setFieldValue(idNameLeft, availableIds);
         formik.setFieldValue(idNameRight, []);
         return;
      }

      // Si los datos están vacíos, vaciar todo
      if (availableIds.length === 0) {
         setLeft([]);
         setRight([]);
         setChecked([]);
         formik.setFieldValue(idNameLeft, []);
         formik.setFieldValue(idNameRight, []);
         return;
      }

      // Sincronización normal: filtrar IDs inválidos
      const currentFormLeft = (formik.values[idNameLeft] || []) as number[];
      const currentFormRight = (formik.values[idNameRight] || []) as number[];
      ...
      // Determinar si necesitamos inicializar con todos los chips
      const shouldUseAllChips = validLeft.length === 0 && validRight.length === 0;
      const finalLeft = shouldUseAllChips ? availableIds : validLeft;
      const finalRight = shouldUseAllChips ? [] : validRight;
   ...
   }, [data, idNameLeft, idNameRight, formik]);
