...
const StepSeleccion: React.FC<StepSeleccionProps> = ({
  ...
  const filteredProjs: Project[] = projects.filter((p) => {
    const matchSearch =
      ...
      String(p.assigned_district).includes(search.toLowerCase());
      ...
      >
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
          ...
          <div className="voter-badge" style={{ margin: 0 }}>
            ...
            &nbsp;{userAuth?.username ?? "—"} · {voterCasilla}
          </div>
        </div>
        ...
        {dropdownOpen && (
          <div className="projects-dropdown">
            {filteredProjs.length === 0 ? (
              <div style={{ padding: "16px", textAlign: "center", color: "var(--gris-claro)", fontSize: ".85rem" }}>
                Sin resultados
              </div>
            ) : (
              filteredProjs.map((p) => (
