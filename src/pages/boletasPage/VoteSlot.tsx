...
export const VoteSlot: React.FC<VoteSlotProps> = ({ index, project, shake, onRemove }) => {
  ...
  return (
    <div
      className={`vote-slot ${project ? "filled" : "empty"} ${shake ? "shake" : ""}`}
...
export const ProjectOption: React.FC<ProjectOptionProps> = ({ project, selected, disabled, onSelect }) => {
  ...
  return (
    <div
      className={`project-option ${selected || disabled ? "disabled" : ""}`}
      onClick={() => !selected && !disabled && onSelect(project)}
    >
      <div className="project-option-num">{project.id}</div>

      <div style={{ flex: 1 }}>
        <div
          className="project-option-name"
          style={{ color: selected ? "var(--gris)" : "var(--negro)" }}
        >
          {project.project_name}
          {selected && (
            <span style={{ fontSize: ".7rem", color: "var(--guinda)", fontWeight: 700, marginLeft: 6 }}>
              ✓ Elegido
            </span>
          )}
        </div>
